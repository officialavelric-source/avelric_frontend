        ...
        {cards.map((c, i) => (
          <Reveal key={c.eyebrow} delay={i * 0.08}>
            <div className="group relative overflow-hidden rounded-[3px] border border-softblack/10">
              ...
              <div className="pointer-events-none absolute inset-x-0 bottom-0 p-5 sm:p-7 text-ivory md:p-8">
                <p className="label text-[10px] tracking-wider text-ivory/70 sm:text-[11px]">{c.eyebrow}</p>
                <h3 className="mt-1.5 sm:mt-2 max-w-sm font-display text-[20px] leading-[1.15] tracking-[-0.01em] sm:text-[24px] md:text-[30px]">{c.title}</h3>
                <p className="mt-1.5 sm:mt-2.5 max-w-sm text-[12.5px] leading-relaxed text-ivory/80 line-clamp-3 sm:line-clamp-none sm:text-[13.5px]">{c.body}</p>
                <div className="pointer-events-auto mt-3.5 sm:mt-5 flex flex-wrap items-center gap-x-4 sm:gap-x-6 gap-y-2">
                  <Link to={c.to} className="label inline-flex items-center gap-1.5 sm:gap-2 border-b border-ivory/50 pb-0.5 sm:pb-1 text-[10.5px] sm:text-[11px] transition-colors hover:border-ivory">
                    {c.cta}
                    <span aria-hidden="true">→</span>
                  </Link>
                  {c.secondaryTo && (
                    <Link to={c.secondaryTo} className="label text-[10.5px] sm:text-[11px] text-ivory/65 transition-colors hover:text-ivory">
                      {c.secondaryLabel}
                    </Link>
                  )}
                </div>
